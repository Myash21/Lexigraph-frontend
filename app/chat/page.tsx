...
import { useChat } from "@/lib/app-context";
...
export default function ChatPage() {
    ...
    const handleSend = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await simulateTyping(data.answer, assistantId, data.sources);
        } catch {
                        ...
                        {messages.map((message) => (
                            <div key={message.id} className={`flex gap-4 ${message.role === "user" ? "flex-row-reverse" : ""}`}>
                                ...
                                <div className={`flex flex-col max-w-[80%] ${message.role === "user" ? "items-end" : "items-start"}`}>
                                    ...
                                    {message.role === "assistant" &&
                                        message.sources &&
                                        (message.sources.vector.length > 0 || message.sources.graph.length > 0) && (
                                            <div className="mt-3 space-y-3 w-full animate-in fade-in duration-500">
                                                {message.sources.vector.length > 0 && (
                                                    <Card className="bg-background shadow-sm overflow-hidden border-blue-500/20">
                                                        <CardContent className="p-0">
                                                            <div className="bg-blue-500/10 px-3 py-1.5 flex items-center justify-between border-b border-blue-500/10">
                                                ...
                                                {message.sources.graph.length > 0 && (
                                                    <Card className="bg-background shadow-sm overflow-hidden border-purple-500/20">
                                                        <CardContent className="p-0">
                                                            <div className="bg-purple-500/10 px-3 py-1.5 flex items-center border-b border-purple-500/10">
